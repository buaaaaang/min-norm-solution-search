...
class Teacher(nn.Module):
    def __init__(self):
        ...
        with torch.no_grad():
            for i in range(3):
                self.layer2.weight[0,i] = self.layer2.weight[0,i] / ((w1[i][0] ** 2 + w1[i][1] ** 2) ** 0.5 * abs(self.layer2.weight[0,i]))
...
class Student(BaseModel):
    def __init__(self):
        ...
        n_train_data = 15
        with torch.no_grad():
            self.train_input = nn.functional.normalize(torch.randn((n_train_data,2)))
        ...
        for i in range(student_weight1.shape[0]):
            if student_weight2[0][i] > 0:
                plt.scatter(student_weight1[i][0] * abs(student_weight2[0][i]) * enlarge,
                            student_weight1[i][1] * abs(student_weight2[0][i]) * enlarge,
                            color='red', s=5)
            else:
                plt.scatter(student_weight1[i][0] * abs(student_weight2[0][i]) * enlarge,
                            student_weight1[i][1] * abs(student_weight2[0][i]) * enlarge,
